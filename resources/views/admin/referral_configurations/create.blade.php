...
@section('title', 'Create Referral Configuration')

@section('content_header')
    <h1>Create Referral Configuration</h1>
@stop

@section('content')
    <div class="card">
        <div class="card-body">
            <form action="{{ route('admin.referral-configurations.store') }}" method="POST" id="configForm">
                @csrf
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="number" step="0.01" min="0" class="form-control @error('total_allocation') is-invalid @enderror" id="total_allocation" name="total_allocation" value="{{ old('total_allocation') }}" required>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <select class="form-control @error('max_level') is-invalid @enderror" id="max_level" name="max_level" required>
                                @for ($i = 1; $i <= 11; $i++)
                                    <option value="{{ $i }}" {{ old('max_level') == $i ? 'selected' : '' }}>{{ $i }}</option>
                                @endfor
                            </select>
                            @error('max_level')
                                <div class="invalid-feedback">{{ $message }}</div>
                    ...
                    <div class="col-md-4 mb-3">
                        <div class="form-group">
                            ...
                            <input type="number" step="0.01" min="0" class="form-control level-bonus" 
                                id="level_${i}" name="level_bonuses[${i}]" placeholder="Auto">
                        </div>
                    </div>
                `);
